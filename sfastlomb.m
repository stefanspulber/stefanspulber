function [P,f,alpha] =sfastlomb(x,t,varargin)
...
%
% SYNTAX
%   [P,f,alpha] = fastlomb(x,t,hifac,ofac,a);
%   [P,f,alpha] = fastlomb(x,t,hifac,ofac);
%   [P,f,alpha] = fastlomb(x,t,hifac);
...
[x,t,hifac,ofac,a_usr,f] = init(x,t,varargin{:});
nt = length(t);
...
%% init (initialize) 
function [x,t,hifac,ofac,a,f] = init(x,t,varargin)
    if nargin < 5, a = [];    % set default value for a 
    else           a = sort(varargin{3}); 
                   a = a(:)';
    end
    if nargin < 4, ofac = 4;  % set default value for ofac  
    else           ofac = varargin{2};
    end
    if nargin < 3, hifac = 1; % set default value for hifac 
    else           hifac = varargin{1};
    end
